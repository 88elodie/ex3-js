 ...
 * ?tri=[date, nom]
 * ?ordre=[asc, desc]
 * Ex 3 : /maires/liste?tri=date&ordre=asc
 * Ex 4 : /maires/liste?tri=nom&ordre=desc
 * 
 */
...
export default class MaireFetch {
     ...
     */
    constructor(){
      fetch(this.URL+"nombre")
        .then(data=>data.json())
        .then(data=>{
          this.nbMaire = data.length;
        });
    }
    ...
    rechercheMaires(params){
      let url = this.URL+`recherche?type=${params.type}&valeur=${params.valeur}`;
      fetch(url)
        .then(data=>data.json())
        .then(data=>{
     ...
     */
    listeMaires(params) {
      let url = this.URL+`liste?tri=${params.type}&ordre=${params.ordre}`;
      fetch(url)
        .then(data=>data.json())
        .then(data=>{
          params.callback(data);
